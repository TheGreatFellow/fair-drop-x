...
import { isNullifierUsed, signVoucher } from "@/lib/voucher";

/**
 * POST { buyer, result } -> { voucher, signature }
 *
 * `result` is the IDKit widget's output, forwarded untouched. On a valid proof for this buyer,
 * returns an EIP-712 voucher the buyer passes to Drop.buy().
 *
 *   200  voucher issued
...
export async function POST(request: Request) {
  let body: { buyer?: unknown; result?: IDKitResult };
  try {
  ...
  try {
    ...
    const { voucher, signature } = await signVoucher(buyer, nullifier);
